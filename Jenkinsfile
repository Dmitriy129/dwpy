pipeline {
    ...
    stages {
        ...
        stage('run mock script1') {
            steps {
                withCredentials([string(credentialsId: 'GitHubAccessToken', variable: 'GITHUB_ACCESS_TOKEN'), string(credentialsId: 'MoodleccessToken', variable: 'MOODLE_ACCESS_TOKEN') ]){
